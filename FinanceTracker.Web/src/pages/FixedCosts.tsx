...
export default function FixedCosts() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Fehler beim Laden der Fixkosten.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDeleteFixedCost = async () => {
    ...
    try {
      ...
      setError(err.message || 'Fehler beim Löschen der Fixkosten-Regel.');
    }
  };
  ...
  const handleManualProcess = async () => {
    ...
    try {
      ...
      setError(err.message || 'Fehler beim Ausführen des Fixkosten-Checks.');
    } finally {
      setProcessing(false);
    }
  };

  const totalMonthlySum = useMemo(() => {
    return fixedCosts
  ...
  const activeCount = useMemo(() => {
    return fixedCosts.filter((fc) => fc.isActive).length;
  }, [fixedCosts]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-dark-400 mt-1">
            Verwalte deine wiederkehrenden Fixkosten. Fällige Fixkosten werden automatisch als Transaktion verbucht.
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            title="Prüft sofort ob fällige Fixkosten als Transaktion verbucht werden müssen"
          >
      ...
      {/* Info Banner explaining "Jetzt prüfen" */}
      <div className="p-4 bg-primary-500/10 border border-primary-500/30 rounded-2xl text-xs text-primary-200 flex items-start gap-3">
        ...
        <div>
          <span className="font-semibold text-white block mb-0.5">Automatisches & Manuelles Prüfen der Fixkosten</span>
          Fixkosten werden automatisch 1x täglich im Hintergrund sowie bei jedem Start der Anwendung geprüft. Wenn eine Regel fällig ist, wird die Buchung automatisch in den Transaktionen angelegt. Über den Button <strong className="text-white">„Jetzt prüfen“</strong> kannst du diesen Check jederzeit manuell ausführen.
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KpiCard
          title="Monatliche Fixkosten Gesamt"
          value={fmt(totalMonthlySum)}
          icon={<Repeat className="h-5 w-5" />}
          color="red"
        />
        <KpiCard
          title="Aktive Fixkosten-Regeln"
          value={String(activeCount)}
          icon={<CheckCircle2 className="h-5 w-5" />}
          color="blue"
        />
        <KpiCard
          title="Gesamtanzahl Regeln"
      ...
      <div className="bg-dark-900 border border-dark-800 rounded-2xl p-6 shadow-xl">
        <h3 className="text-lg font-semibold text-white mb-4">Eingerichtete Fixkosten</h3>

        {loading ? (
          <div className="p-12 text-center text-dark-400">
            <RefreshCw className="h-6 w-6 animate-spin mx-auto mb-2 text-primary-500" />
            <p className="text-sm">Lade Fixkosten aus dem Backend…</p>
          </div>
        ) : fixedCosts.length === 0 ? (
          <div className="text-center py-12 text-dark-400">
            <Repeat className="h-10 w-10 mx-auto mb-3 opacity-30 text-primary-400" />
            <p className="text-base font-medium text-white mb-1">Noch keine Fixkosten eingerichtet</p>
            <p className="text-xs text-dark-400 max-w-sm mx-auto mb-4">
              Lege deine wiederkehrenden Ausgaben (z. B. Miete, Abos, Versicherungen) an, damit sie am Fälligkeitstag automatisch verbucht werden.
            </p>
            <button
              onClick={handleOpenAddModal}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {fixedCosts.map((fc) => (
                  ...
                  >
                    <td className="py-3.5 px-4 font-medium text-white">{fc.note || 'Fixkosten'}</td>
                    ...
                    <td className="py-3.5 px-4 text-right font-bold text-red-400">{fmt(fc.amount)}</td>
      ...
      {deletingFixedCost && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-dark-900 border border-dark-700 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-2">Fixkosten-Regel löschen?</h3>
            <p className="text-xs text-dark-300 mb-6">
              Möchtest du die Regel für "{deletingFixedCost.note || 'Fixkosten'}" wirklich löschen? Bereits generierte Transaktionen bleiben dabei erhalten.
            </p>
